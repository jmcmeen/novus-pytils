...
from novus_pytils.files.core import (
    download_file, get_file_name, get_file_extension, get_file_size, 
    file_exists, copy_file, move_file, delete_file, create_file_from_content,
    read_file_content, append_to_file, get_file_creation_time,
    get_file_modification_time, is_file_empty, filter_files_by_size, 
    filter_files_by_date, rename_file, get_file_permissions, 
    set_file_permissions, create_backup, restore_backup,
    get_files_by_extension, create_directory, delete_directory, 
    get_directory_size, count_files_in_directory, get_subdirectories, 
    get_files_recursively, sync_directories, read_file_bytes, read_file_text
)
...
class TestFileCore:
    ...
    def test_get_file_size(self):
        ...
        try:
            # Act
            result = get_file_size(temp_file_path)
    ...
    def test_get_file_size_mock(self, mock_getsize):
        """Test getting file size with mock."""
        # Act
        result = get_file_size('/test/path')

        # Assert
        assert result == 1024
        mock_getsize.assert_called_once_with('/test/path')

    def test_get_file_size_file_not_found(self):
        """Test getting size of non-existent file."""
        # Act & Assert
        with pytest.raises(FileNotFoundError):
            get_file_size('/nonexistent/path')

    def test_get_file_size_pathlib_path(self):
        """Test getting file size with pathlib Path object."""
        # Arrange
        test_content = b'pathlib size test'
        expected_size = len(test_content)
        with tempfile.NamedTemporaryFile(delete=False) as temp_file:
            temp_file.write(test_content)
            temp_file_path = Path(temp_file.name)
    ...
    def test_get_file_size_large_content(self):
        """Test getting size of file with larger content."""
        # Arrange
        test_content = b'X' * 5000  # 5000 bytes
        expected_size = 5000
        with tempfile.NamedTemporaryFile(delete=False) as temp_file:
            temp_file.write(test_content)
            temp_file_path = temp_file.name
...
class TestDownloadFile:
    ...
    @patch('novus_pytils.files.core.requests.get')
